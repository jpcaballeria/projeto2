    ...
    window.theme = {
        ...
        getShippingRates: function(){
            ...
            $('.shipping-form').on('submit', function(event){
                ...
                /* Validate zip code first using Brasil API web service */
                $.ajax({
        ...
        detectProductVariantChanges: function(){
            ...
            $('.product-variants, .product-additional-fields').on('change', '.infoSelects', function(){
        ...
        validGift: function(){
            ...
            const brindes = $('.product-gifts .brinde_produto').length;
            if(brindes == 1){
                $('#form_comprar').on('submit', function(){
                    let variantBrinde  = $('#form_comprar').find('input[type="hidden"][name="variacao"]');
                    let errorBrinde = $('#span_erro_carrinho').text();
                    
                    if ( (variantBrinde.length && variantBrinde.val() === '') && (errorBrinde === '') ){
                        $('#span_erro_carrinho').text('Selecione uma op\u00e7\u00e3o para varia\u00e7\u00e3o do produto');
                    }
                })    
            }            
